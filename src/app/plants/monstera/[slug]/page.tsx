...
export default function PlantPage({ params }: PageProps) {
  ...
  const morphEntries = Object.entries(morphology);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-10 gap-8 max-w-7xl mx-auto px-4 py-8 bg-background">
      ...
      <div className="lg:col-span-7 space-y-8">
        ...
        <nav className="flex items-center gap-2 text-xs text-muted">
          ...
          <span>/</span>
          <span className="text-heading">{scientificName}</span>
        </nav>
        ...
        <div className="rounded-xl bg-card p-5">
          ...
          <p className="mt-3 text-xs text-muted">
            Typical mature leaf size: 60–90 cm in length
          </p>
        </div>
            ...
            <div className="rounded-lg bg-card/60 p-4 border border-card/80">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-heading font-medium">{origin}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-muted">Humidity</span>
                  <span className="text-heading font-medium">{quickFacts.humidity}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-muted">Temperature</span>
                  <span className="text-heading font-medium">{quickFacts.temperature}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-muted">Light</span>
                  <span className="text-heading font-medium">{quickFacts.light}</span>
                </div>
                <div className="flex flex-col gap-1">
        ...
        <div className="sticky top-24 space-y-6">
          ...
          <div className="rounded-xl bg-card p-5">
            ...
            <p className="mb-4 text-xs text-muted">
              Median GBP — Last {priceHistory.length} months
            </p>
            <PriceChart
              data={priceHistory}
            ...
            <div className="mt-3 flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider ${
                  marketMetrics.marketStatus === "Volatile"
                    ? "bg-rarity/10 text-rarity"
                    : marketMetrics.marketStatus === "Rising"
                    ? "bg-green-500/10 text-green-400"
                    : marketMetrics.marketStatus === "Declining"
              ...
              >
                <svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                  ...
                  ) : marketMetrics.marketStatus === "Declining" || marketMetrics.marketStatus === "Volatile" ? (
                ...
                {marketMetrics.marketStatus}
              </span>
              <span className="text-xs text-muted">
                {marketMetrics.threeMonthChangePercent > 0 ? "+" : ""}
                {marketMetrics.threeMonthChangePercent.toFixed(1)}% 3mo
              </span>
            </div>
          </div>
      ...
      {/* ===== BOTTOM FOOTPRINT: Recommended Plants (Cols 1-10) ===== */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
          {recommendedPlants.map((plant) => (
            <Link
              key={plant.slug}
              href={
                plant.slug === "burle-marx-flame"
                  ? `/plants/monstera/${plant.slug}`
                  : `/plants/philodendron/${plant.slug}`
              }
